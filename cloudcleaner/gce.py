...
def get_categorized_deployments(gce_wrapper: gce.GceWrapper, now, users):
...
class GceCleaner(AbstractCleaner):
    @property
    def gce_wrapper(self):
        ...
        return gce.GceWrapper(json.loads(credentials), credentials_path)

    def collect_resources(self, users):
        self.categorized_deployments = get_categorized_deployments(self.gce_wrapper, self.now, users)

    def clean_resources(self, dry_run, match_str):
        self.actions = perform_deployment_actions(self.categorized_deployments)

    def make_report(self):
        return make_json_report(self.now, self.categorized_deployments, self.actions)
